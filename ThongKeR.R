...
hist(data$p_price,col = 'pink',breaks=50,xlab = "Kho???ng giá s???n ph???m", ylab = "S??? lu???ng s???n ph???m", main = "Bi???u d??? th??? hi???n các kho???ng giá s???n ph???m")
#----------------------------------------------------------------
...
hist(data$p_rating,col = 'red',breaks=70,xlab = "Dánh giá trung bình c???a s???n ph???m (%)", ylab = "S??? lu???ng dánh giá", main = "Bi???u d??? th??? hi???n t???ng quan dánh giá s???n ph???m trên Lazada")
#----------------------------------------------------------------
# Tên shop bán hàng
mode(data$s_name)
...
fig
#----------------------------------------------------------------
# ???nh hu???ng c???a vi???c ph???n h???i khách hàng và th???i gian gian hàng d???n dánh giá s???n ph???m c???a khách hàng
fig <- plot_ly(x=data$s_ship_ontime, y=data$s_response_rate, z=data$s_rating, type = "contour",
               colorscale = 'Jet',
               autocontour = F
)%>% layout(title = '???nh hu???ng c???a vi???c ph???n h???i khách hàng và th???i gian gian hàng d???n dánh giá s???n ph???m c???a khách hàng',
            yaxis = list(zeroline = FALSE),
            xaxis = list(zeroline = FALSE))
fig
#----------------------------------------------------------------
# ???nh hu???ng c???a s???n ph???m chính hãng và không chính hãng d???n ch???t lu???ng s???n ph???m
...
fig
#----------------------------------------------------------------
# Ph???n tram s???n ph???m khách hàng quan tâm
fig <- plot_ly(labels = data$p_cate, values = data$p_number_reviews, type = 'pie',
               ...
               marker = list(colors = colors,
                             line = list(color = '#FFFFFF', width = 1)),
               showlegend = FALSE)%>% layout(title = 'Ph???n tram danh m???c s???n ph???m du???c khách hàng tiêu th???',
                                             xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
                                             yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE))
fig